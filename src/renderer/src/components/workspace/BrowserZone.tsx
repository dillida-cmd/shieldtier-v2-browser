...
export function BrowserZone() {
  const { navCanGoBack, navCanGoForward, navIsLoading, navCurrentUrl, navTitle } = useStore();
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--st-bg-primary)]">
      <div className={`flex items-center h-10 px-2 gap-2 border-b flex-shrink-0 bg-[var(--st-bg-panel)] transition-colors ${navIsLoading ? 'border-b-[var(--st-accent)]' : 'border-b-[var(--st-border)]'}`}>
        <div className="flex items-center gap-1 flex-shrink-0">
          ...
          <NavButton label={navIsLoading ? 'Stop loading' : 'Refresh page'} onClick={reload}>
            {navIsLoading ? (
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <span><Badge severity="info" className="flex-shrink-0">SANDBOXED</Badge></span>
          </TooltipTrigger>
          <TooltipContent>All traffic routed through sandbox isolation</TooltipContent>
        </Tooltip>

        {navTitle && (
          <span className="text-[var(--st-text-label)] text-[10px] truncate max-w-40 flex-shrink-0">{navTitle}</span>
        )}
      ...
      {!navCurrentUrl || navCurrentUrl === 'about:blank' || navCurrentUrl.startsWith('shieldtier://') ? (
        <div className="flex-1 flex items-center justify-center bg-[var(--st-bg-primary)]">
          <div className="flex flex-col items-center gap-3 text-[var(--st-text-muted)]">
            <svg width="52" height="52" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" opacity="0.3">
              <circle cx="12" cy="12" r="10" />
              <path d="M2 12h20M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z" />
            </svg>
            <span className="text-[11px] animate-pulse">Navigate to a URL to begin analysis</span>
          </div>
        </div>
      ) : (
